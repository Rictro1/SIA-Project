...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($kritik_saran)) {
      $stmt = $conn->prepare("INSERT INTO kritik_saran (mhs_id, mhs_name, kritik_saran) VALUES (?,?,?)");
      $stmt->bind_param("iss", $mhs_id, $mhs_name, $kritik_saran);
  
      if ($stmt->execute()) {
        $success_message = "Kritik dan Saran anda berhasil disimpan!";
      } else {
      ...
      $stmt->close();
        ...
        $error_message = "Kritik dan Saran tidak boleh kosong!";
    }
}
?>
...
<nav id="sidebar">
    ...
    <div class="project">
        <form action="" method="post" class="fade-in">
            <h2 for="kritik_saran">Kritik dan Saran</h2>
        ...
        <?php endif; ?>
            ...
            <div class="note">
              <h3>Catatan :</h3>
              <p class="info">Isilah kotak kritik dan saran sebaik-baiknya dan sejujur-jujurnya demi kebaikan sistem dan pelayanan</p>
            </div>
            <textarea name="kritik_saran" class="input-saran" required placeholder="Isi kolom dengan saran yang ingin Anda kirim"></textarea>
