...
$query_mhs = "SELECT mhs_id, mhs_name, semester, program_studi FROM data_mhs WHERE mhs_id = ?";
$stmt = $conn->prepare($query_mhs);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query_check = "SELECT * FROM mhs_matkul WHERE mhs_id = ? AND matkul_id = ?";
    $stmt_check = $conn->prepare($query_check);
    $stmt_check->bind_param("ii", $mhs_id, $matkul_id);
    $stmt_check->execute();
    $result_check = $stmt_check->get_result();

    if ($result_check->num_rows > 0) {
        echo "<script>showAlert('Anda sudah mengambil mata kuliah ini.', 'error');</script>";
    } else {
        $query = "SELECT * FROM matkul_list WHERE matkul_id = ?";
        ...
        if ($matkul) {
            $insert = "INSERT INTO mhs_matkul (mhs_id, mhs_name, matkul_id, matkul_name, dosen_id, dosen_name, sks, hari, pukul) 
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($insert);
            $stmt->bind_param(
                "isississs",
                $mhs_id, $mhs_name, 
                $matkul['matkul_id'], $matkul['matkul_name'], 
                $matkul['dosen_id'], $matkul['dosen_name'], 
                $matkul['sks'], $matkul['hari'], $matkul['pukul']
            );

            if ($stmt->execute()) {
                $success_message = "KRS berhasil disimpan!";
            } else {
                $error_message = "Terjadi kesalahan saat menyimpan KRS!";
            }
        }
    }
}

$query_matkul = "SELECT * FROM matkul_list";
$result_matkul = $conn->query($query_matkul);

$query_krs = "SELECT * FROM mhs_matkul WHERE mhs_id = ?";
$stmt = $conn->prepare($query_krs);
$stmt->bind_param("i", $mhs_id);
$stmt->execute();
$result_krs = $stmt->get_result();
?>
...
@media print {
    ...
    .krs::after {
        content: "Kartu Rencana Studi (KRS) - Cetakan Resmi";
        display: block;
        text-align: center;
        font-size: 18px;
        font-weight: bold;
...
<nav id="sidebar">
    <ul>
      ...
      <li>
        ...
        <ul class="sub-menu">
          <div>
            <li  class="active"><a href="krs.php">KRS</a></li>
  ...
  <div class="krs">
    ...
    <form method="POST">
        ...
        <select name="matkul_id" required>
            ...
            <?php while ($row = $result_matkul->fetch_assoc()): ?>
                <option value="<?= $row['matkul_id'] ?>">
                    <?= htmlspecialchars($row['matkul_name']) ?> - <?= htmlspecialchars($row['dosen_name']) ?> (<?= $row['sks'] ?> SKS) - <?= $row['hari'] ?> <?= $row['pukul'] ?>
                </option>
            <?php endwhile; ?>
        </select>
    ...
    <div class="info">
            <div class="info-item">
                <div class="row">
                    ...
                    <div class="value">: <?= htmlspecialchars($mahasiswa['mhs_id']) ?></div>
                </div>
                <div class="row">
                    <div class="label">Nama</div>
                    <div class="value">: <?= htmlspecialchars($mahasiswa['mhs_name']) ?></div>
                </div>
                <div class="row">
                    <div class="label">Semester</div>
                    <div class="value">: <?= htmlspecialchars($mahasiswa['semester']) ?></div>
                </div>
                <div class="row">
                    <div class="label">Program Studi</div>
                    <div class="value">: <?= htmlspecialchars($mahasiswa['program_studi']) ?></div>
                </div>
            </div>
        </div>
    <table>
        ...
        <?php $no = 1; while ($row = $result_krs->fetch_assoc()): ?>
        <tr>
            <td><?= $no++ ?></td>
    ...
    <button onclick="cetakKRS()" class="print-btn">Cetak KRS</button>
</div>
