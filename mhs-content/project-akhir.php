...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $judul_projek = trim($_POST['judul_projek']);

    if (!empty($judul_projek)) {
      $stmt = $conn->prepare("INSERT INTO projek_akhir (mhs_id, mhs_name, judul) VALUES (?,?,?)");
      $stmt->bind_param("iss", $mhs_id, $mhs_name, $judul_projek);
  
      if ($stmt->execute()) {
          $success_message = "Judul proyek berhasil disimpan!";
      } else {
      ...
      $error_message = "Judul proyek tidak boleh kosong!.";
    }
}
?>
...
<head>
    ...
    <title>Input Judul Projek Akhir</title>
    ...
    input:focus {
        border-color:#F25C00;
        box-shadow: 0px 0px 5px rgba(228, 221, 7, 0.69);
        outline: none;
    }
...
<nav id="sidebar">
    <ul>
      ...
      <li>
        ...
        <ul class="sub-menu">
          <div>
            <li class="active"><a href="project-akhir.php">Projek Akhir (M)</a></li>
    ...
    <div class="project">
        <form action="" method="post" class="fade-in">
            <h2 for="judul_projek">Judul Projek</h2>
        ...
        <?php endif; ?>
            ...
            <div class="note">
              <h3>Catatan :</h3>
              <p class="info">Pastikan judul proyek yang Anda masukkan sesuai dengan tema yang telah ditentukan.</p>
              <p class="info">Judul proyek yang valid akan membantu dalam proses bimbingan dan penelitian lebih lanjut.</p>
            </div>
            <input type="text" name="judul_projek" id="judul_projek" required placeholder="Isi kolom dengan judul projek yang ingin Anda kirim">
