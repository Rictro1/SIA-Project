...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $matkul_id = isset($_POST['matkul_id']) ? trim($_POST['matkul_id']) : ''; 
    $matkul_name = isset($_POST['matkul_name']) ? trim($_POST['matkul_name']) : '';
    $dosen_id = isset($_POST['dosen_id']) ? trim($_POST['dosen_id']) : '';
    
    if (!isset($_POST['dosen_name']) || empty($_POST['dosen_name'])) {
        $dosen_query = $conn->prepare("SELECT dosen_name FROM data_dosen WHERE dosen_id = ?");
        $dosen_query->bind_param("i", $dosen_id);
        $dosen_query->execute();
        $dosen_query->bind_result($dosen_name);
        $dosen_query->fetch();
        $dosen_query->close();
    } else {
        $dosen_name = trim($_POST['dosen_name'] ?? '');
    }

    $sks = isset($_POST['sks']) ? intval($_POST['sks']) : 0;
    $hari = isset($_POST['hari']) ? trim($_POST['hari']) : '';
    $pukul = isset($_POST['pukul']) ? trim($_POST['pukul']) : '';

    if (empty($matkul_id) || empty($matkul_name) || empty($dosen_id) || empty($sks) || empty($hari) || empty($pukul)) {
        $error_message = "Semua kolom harus diisi!";
    } else {
        $sql = "UPDATE matkul_list SET 
                    matkul_name = ?, 
                    dosen_id = ?, 
                    dosen_name = ?, 
                    sks = ?, 
        ...
        if ($stmt->execute()) {
            $success_message = "Data mata kuliah berhasil diperbarui!";
        } else {
...
<nav id="sidebar">
  ...
  <div class="container">
    ...
    <form method="POST" action="">
        ...
        <label for="matkul_id">ID Mata Kuliah :</label>
        <input type="text" name="matkul_id" required><br>

        <label for="matkul_name">Nama Mata Kuliah :</label>
        <input type="text" name="matkul_name" required><br>
        ...
        <input type="text" name="pukul" required><br>
        
        <button type="submit" class="submit-button">Simpan Data</button>
        <a href="matkul_list.php?logout=true" class="submit-button">Back</a>
    </form>
</div>
</body>
</html>
