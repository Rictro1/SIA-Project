...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        $sql = "SELECT password FROM data_dosen WHERE dosen_id = ?";
        ...
        if ($user && $current_password === $user['password']) {
            $update_sql = "UPDATE data_dosen SET password = ? WHERE dosen_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("si", $new_password, $dosen_id);
  ...
  <div class="update-pass-container">
    <div class="form-card">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn">Ubah Password</button></br>
            <button  class="btn"><a href="account.php?logout=true">Back</a></button>
        </form>
    </div>
</div>
</body>
